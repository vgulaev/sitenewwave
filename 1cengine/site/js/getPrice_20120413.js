...
function getNodesMSIE(DR, RootUl, level, Price){
	...
	while(DR.childNodes[i]!=null){
		if(oro.tagName == "m:Группа"){
            ...
            var itemName = RootUl.parentNode.getAttribute('id')
			$(NodeLi).append("<span class='itemName' style='padding-left:"+level*15+"px;'>"+oro.childNodes[0].childNodes[0].data+"</span> <div class='pr'><a href='Добавить товар' class='addItem' onClick=\"addItem('"+itemName+"', '"+oro.childNodes[0].childNodes[0].data+"', '"+BasePrice+"', '"+kfArray+"', '"+oro.childNodes[2].childNodes[0].data+"', '"+oro.childNodes[3].childNodes[0].data+"', '"+oro.childNodes[4].childNodes[0].data+"'); return false\">купить</a>"+prices+"</div>")
			orob = oro	
			RootUl.appendChild(NodeLi)
			RootUl.parentNode.setAttribute("class","itemGroup")
		}
