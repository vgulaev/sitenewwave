...
(function() {
  ...
  App.show_rezka_ch_modal = function() {
    var checked, d_name, i, item, len, ref, ref1, table, table_rows;
    table_rows = "";
    ref = App.MyBasket._item_list;
    for (i = 0, len = ref.length; i < len; i++) {
      item = ref[i];
      if (item.is_measureable()) {
        if (ref1 = item.id, indexOf.call(App.MyBasket._rezka_list, ref1) >= 0) {
          checked = "checked";
        } else {
          checked = "";
        }
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      if (ref1 = item.id, indexOf.call(App.MyBasket._rezka_list, ref1) >= 0) {
        if (!$("div.rezka_item").is("[rid='" + item.id + "']")) {
          results.push(add_rezka_item(item));
        } else {
          results.push(void 0);
        }
      } else {
        if ($("div.rezka_item").is("[rid='" + item.id + "']")) {
          results.push(delete_rezka_item(item.id));
        } else {
          results.push(void 0);
        }
      }
  ...
  fill_rezka = function(itid) {
    var M_count, M_len, _item, i, item, len, ref, results, rezka_array;
    ref = App.MyBasket._item_list;
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      _item = ref[i];
