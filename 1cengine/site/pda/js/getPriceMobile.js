...
function getNodes(DR, RootUl, level, Price){
	...
	while(i < DR.childNodes.length){
		...
		if(oro.tagName == "m:Группа"){
			...
			$(NodeLi).append("<li class='UlName' onclick='$(this).next().slideToggle(0)' ><span style='padding-left:"+level*15+"px;'>"+oro.childNodes[1].childNodes[0].data+"</span></li>")
			NodeLi.setAttribute("id", oro.childNodes[1].childNodes[0].data)

			var NodeUl = document.createElement("ul")
			NodeUl.setAttribute("style","display:none")
            ...
            var hline = new Array(level+1).join("<span class='hline'></span>")
			$(NodeLi).append("<span class='itemName' style='padding-left:"+level*15+"px;'>"+oro.childNodes[1].childNodes[0].data+"</span> <div class='pr'>"+prices+"</div>")
