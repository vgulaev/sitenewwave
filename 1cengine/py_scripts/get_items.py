...
def getItems(req):
    ...
    if "strict" in get:
        condition = condition + \
            "CONCAT(display_name, ' ', char_name) LIKE '%" + req + "%' AND "
...
def showItems(req):
    ...
    for row in r:
        ...
        if not row[4] in parentArray:
            ...
            print '<tr class="iHeader"><td><strong>' \
                + row[4] + '</strong></td><td>Размер</td>'
            ...
            for priceType in priceTypeArray:
                if priceType != '':
                    if i == 0:
                        print '<td class="priceHeader">' \
                            + priceType + '<br /><span>Цена \
                            <font color="red">Я</font>ндекса</span></td>'
                    else:
                        print '<td class="priceHeader">' \
                            + priceType + '<br /><span>Цена</span></td>'
                    i = i + 1
        ...
        if not row[8] == 0:
            rt = rt + """<a class="bItem" href="Добавить в корзину"
                    onClick="yaCounter15882208.reachGoal(
                    'onBuyLinkPressed', 'купить');
                    openItem('""" + row[5] + ":" + row[7] + "', '"\
                    + row[6] + "', '" + row[2] + """', '1');
        ...
        if not ralColor == "":
            rt = rt + '<td name="' + row[1] + '" \
                class="itemChar" itemprop="model" \
                style="background-color:' + ralColor + ';">\
                <span style="color:#cfcfcf;\
                text-shadow: 1px 1px 2px black, 0 0 1em grey;">' + \
                row[1] + '</span></td>'
        else:
            rt = rt + '<td name="' + \
                row[1] + '" class="itemChar" itemprop="model">' + \
                row[1] + '</td>'
        ...
        for price in priceArray:
            if price != "":
                if j == paLength:
                    ...
                    rt = rt + '<td class="price itemPrice' + \
                        str(j) + '"><span>' + str(price) + '</span></td>'
...
def getItemByHash(hash):
    ...
    r = connector.dbExecute("""
            SELECT `offers`.`display_name`, `offers`.`char_name`,
            `offers`.`price`, `offers`.`edIzm`
            FROM `offers`
            WHERE `offers`.`hash`='""" + itemHash + """'
            AND `offers`.`father_hash`='""" + pHash + """'
        """)
...
def showItemByHash(hash, char, count, rezka):
    ...
    for row in r:
        cell = """
            ...
            <a href="Убрать из корзины" onClick="delModernItem(
            '""" + hash + """'); return false">X</a>
            <a href="Изменить" onClick="modern_editItem(
            '""" + hash + """'); return false">
            <img src="edit.png" /></a></span></td>
            <td class='itemCharTd'>""" + char + """</td>
            <td class='itemCountTd'><input class='itemCountInput'
            name='""" + row[3] + """' type='textarea'
            value='""" + count + """' disabled /></td>
            <td class='itemEdIzmTd' name='""" + row[3] + """'>
            """ + row[3] + """</td>
            <td class='itemPriceTd' name='""" + row[2] + """'></td>
            <td class='itemNdsKfTd'>18%</td>
            <td class='itemNdsSumTd'></td>
            <td class='itemSumTd'></td>
            <td class='itemRezlaTd' style='display:none'>
            """ + rezka + """</td></tr>
        """
...
if from_hash is True:
    ...
    if "strict" in get and res.__len__() == 0:
        print "<tr><td>Извините, данный товар в настоящее время \
            отсутствует на складе</td></tr>"
